...
command -v cargo >/dev/null 2>&1 || { echo "red-rust-fmt: cargo not installed — skipping" >&2; exit 0; }

# Run from the repo root (AFK invokes hooks with the worktree as CWD).
crate_dir="apps/desktop/src-tauri"
[ -d "$crate_dir" ] || { echo "red-rust-fmt: $crate_dir not found — skipping" >&2; exit 0; }

echo "red-rust-fmt: cargo fmt --all --check in $crate_dir" >&2
cd "$crate_dir"
cargo fmt --all --check 1>&2

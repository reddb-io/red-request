...
- **Token request** — an auth method where a designated Request in the same Collection
  is the token source: its response yields `access_token` (and optionally
  `refresh_token`) via configured extraction paths, and the resolved auth dispatches as
  a bearer. Participates in the scope cascade like any other auth method. Expiry is
  resolved in layers: JWT `exp` claim → configured response field (e.g. `expires_in`)
  → manual TTL → unknown (renew only on 401). Renewal is on-demand at dispatch (~30s
  margin) plus a single retry-and-renew on an unexpected 401; an optional designated
  **refresh request** (may use `{{refresh_token}}`) performs it, falling back to
  re-running the login request when absent or failing. Captured tokens are written as
  **Secrets of the active Environment** under configurable names (defaults
  `access_token` / `refresh_token`; dotted names like `myapi.tokens.x` are a valid
  namespace convention), so they are visible in the environments UI and referenceable
  as `{{name}}` — and, like every secret, exported by name only, never by value.
